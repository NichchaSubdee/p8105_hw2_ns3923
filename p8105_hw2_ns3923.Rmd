...
**Description**: 
*pols_df* dataset contains counts of governors (`gov_dem` or `gov_gop`), senators (`sen_dem` or `sen_gop`), and representatives (`rep_dem` or `rep_gop`) by party(i.e., Democratic and Republican) and the party of the sitting president (`president`). *snp_df* contains the S&P monthly closing value (`close`). Lastly, unemployment_df has the monthly unemployment rate (`unemployment`). After cleaning and tidying these three datasets, and combining them by using `month` and `year` as the key variables, the *Final_df* has `r nrow(Final_df)` rows and `r ncol(Final_df)` variables and covers from January 1947 to June 2015.
