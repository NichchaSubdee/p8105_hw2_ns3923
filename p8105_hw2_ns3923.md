...
Wheel*, and *Gwynnda Trash Wheel* sheets, cleaned names, removed
unncessary columns and rows with missing `dumpster`. In order to combine
these three sheets, I create a new variable called `wheel` and combine
them by this key variable. The final dataset, *FinalTrash_df*, has 1033
rows and 15 variables. Some of key variables describes the volumn of
trash `dumpster`, `weight_tons`, and types of trash like `sports_balls`,
`plastic_bottles`, and `cigarette_butts`.

From the available data, Professor Trash Wheel collected 246.74 tons in
total, and Gwynnada collected 18120 cigarette butts in June 2022.
