...
The cleaned *pols_df* dataset \[822 rows x 9 variables\] contains counts
of governors (`gov_dem` or `gov_gop`), senators (`sen_dem` or
`sen_gop`), and representatives (`rep_dem` or `rep_gop`) by party(i.e.,
Democratic and Republican) and the party of the sitting president
(`president`). Also, the cleaned *snp_df* dataset \[787 rows x 3
variables\] contains the S&P monthly closing value (`close`). Lastly,
*unemployment_df* \[816 rows x 3 variables\] has the monthly
unemployment rate (`unemployment`). After combining these three datasets
by using `month` and `year` as key variables, the *Final_df* has 822
rows and 11 variables, starting from January 1947 to June 2015.
...
Wheel*, and *Gwynns Falls Trash Wheel*.
...
*Update_Trashwheel.xlsx*

**Import data**

``` r
MrTrash_df = read_excel("./Update_Trashwheel.xlsx", sheet = "Mr. Trash Wheel", skip = 1)
ProfTrash_df = read_excel("./Update_Trashwheel.xlsx", sheet = "Professor Trash Wheel", skip = 1)
Gwyn_df = read_excel("./Update_Trashwheel.xlsx", sheet = "Gwynns Falls Trash Wheel", skip = 1)
```
...
**Gwynns Falls Trash Wheel Cleaning**
...
I imported the *Mr. Trash Wheel*, *Professor Trash Wheel*, and *Gwynns
Falls Trash Wheel* sheets, cleaned names, and removed unnecessary
columns and rows with missing `dumpster`. Then, I created a new variable
called `wheel` and stacked the three datasets. The final dataset,
*FinalTrash_df*, has 1188 observations and 15 variables. Some key
variables include `dumpster`, `weight_tons`, and types of trash like
`sports_balls`, `plastic_bottles`, and `cigarette_butts`.

From the available data, Professor Trash Wheel collected 282.26 tons in
total, and Gwynns Falls Trash Wheel collected 18,120 cigarette butts in
June 2022.
...
There are 149 unique ZIP codes and 42 neighborhoods.
...
low-activity ZIPs), or places with too little rental activity for Zillow
to publish a series. A few examples are shown below:

``` r
zips_not_zori |> 
    select(zip, borough, neighborhood) |> 
    slice_head(n = 10) |> 
knitr::kable(caption = "**Table 1:** Examples of ZIP codes present in the crosswalk but missing in ZORI")
```
...
**Table 1:** Examples of ZIP codes present in the crosswalk but missing
in ZORI
